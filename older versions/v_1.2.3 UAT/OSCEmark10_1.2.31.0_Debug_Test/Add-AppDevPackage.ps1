...
#
# Back-up DB
# returns 0 if everything is okay or else 1 or 2 with the corresponding message on the console.
#
function BackUpDatabase
{
	...
	$choice = "y";
	# if error equal to 1 and return back
	$dbBackup = 0;
    $appPackageFamilyName = "";
    #$userName = $env:USERPROFILE.Split("\")[-1];
	
	try{
		$packages = Get-AppxPackage | Where-Object {$_.Name -eq $AppName};
		
		if($packages -ne $null){
			foreach($package in $packages){	
				$appPackageFamilyName = $package.PackageFamilyName;
			}
			
			if($appPackageFamilyName -eq ""){
				$dbBackup = 1;
			}
		}
		else{
	...
	# set filepath and filename in order to retrieve the file later on.
	$pathNAME = $env:USERPROFILE + "\AppData\Local\Packages\" + $appPackageFamilyName + "\LocalState";
	$filePath = $env:USERPROFILE + "\AppData\Local\Packages\" + $appPackageFamilyName + "\LocalState\"+$DBName;

	
	write-host "Try to back-up local database from application: " $AppName;
	try
	{
		If (Test-Path $pathNAME ){
			$destinationPath = $env:USERPROFILE + "\Documents";
			Copy-Item -Path $filePath -Destination $destinationPath;
			write-host "Local DB was successfully copied.";
			$dbBackup = 0;
		}
		else{
			write-host "Fresh installation. New database will be created when the application launches.";
			$dbBackup = 1;
		}
	}
	catch{
		...
		#write-host $ErrorMessage;
		while (1){
		$choice = Read-Host "Could not copy the local DB on this machine. Do you want to proceed without making a backup of the database? (y/n)";
...
function BackUpSettings
{
	...
	# set filepath and filename in order to retrieve the file later on.
	$pathNAME = $env:USERPROFILE + "\AppData\Local\Packages\" + $appPackageFamilyName + "\Settings";
	$filePath = $env:USERPROFILE + "\AppData\Local\Packages\" + $appPackageFamilyName + "\Settings\"+$SettingsName;
	
	write-host "Try to back-up settings from application: " $AppName;
	try
	{
		If (Test-Path $pathNAME ){
			$destinationPath = $env:USERPROFILE + "\Documents";
			Copy-Item -Path $filePath -Destination $destinationPath;
			write-host "Settings successfully copied.";
			$settingsBackup = 0;
		}
		else{
			write-host "New settings will be created when the application launches.";
			$settingsBackup = 1;
		}
	}
	catch{
		...
		write-host $ErrorMessage;
		while (1){
		$choice = Read-Host "Could not copy settings on this machine. Do you want to proceed without making a backup of the settings? (y/n)";
			if($choice.ToLower() -eq "y"){
				$settingsBackup = 1;
				break;
			}v
			elseif ($choice.ToLower() -eq "n"){
				$settingsBackup = $settingsBackup + 1;
				break;
			}
		}
	}
...
#
# Restore DB if it was backed-up
# returns 0 if everything is okay or else 1 or 2 with the corresponding message on the console.
#
function RestoreDatabase
{
	...
	$dbRestore= 0;
    $pathNAME = $env:USERPROFILE + "\Documents";
	$filePath = $env:USERPROFILE + "\Documents\"+$DBName;
    $appPackageFamilyName = "";
    #$userName = $env:USERPROFILE.Split("\")[-1];
    ...
    try{
        ...
        write-host "Error while trying to get the destination folder.";
        $dbRestore = 1;
    }
	...
	$destinationPath = $env:USERPROFILE + "\AppData\Local\Packages\" + $appPackageFamilyName + "\LocalState";
	
    write-host "Try to restore local database for application: " $AppName;
	try
	{
		If (Test-Path $pathNAME ){
    		If (Test-Path $destinationPath ){
                Copy-Item -Path $filePath -Destination $destinationPath;
			    write-host "Local DB was successfully restored.";
				Remove-Item -Path $filePath;
			    $dbRestore = 0;
            }
            else
            {
                Write-Host "Could not found the application's (destination) folder physican path. DB could not be restored.";
			    $dbRestore = 1;
            }
		}
		else{
			write-host "Could not found the origin folder.";
			$dbRestore = 1;
		}
	}
    catch{
...
#
# Restore settings if it was backed-up
# returns 0 if everything is okay or else 1 or 2 with the corresponding message on the console.
#
function RestoreSettings
{
	...
	$settingsRestore= 0;
    $pathNAME = $env:USERPROFILE + "\Documents";
	$filePath = $env:USERPROFILE + "\Documents\" + $SettingsName;
    $appPackageFamilyName = "";
    #$userName = $env:USERPROFILE.Split("\")[-1];
    ...
    if ($settingsRestore -eq 1){
        return $settingsRestore;
    }

	$destinationPath = $env:USERPROFILE + "\AppData\Local\Packages\" + $appPackageFamilyName + "\Settings";
	
    write-host "Try to restore settings for application: " $AppName;
	try
	{
		If (Test-Path $pathNAME ){
    		If (Test-Path $destinationPath ){
                Copy-Item -Path $filePath -Destination $destinationPath;
			    write-host "Settings were successfully restored.";
				Remove-Item -Path $filePath;
			    $settingsRestore = 0;
            }
            else
            {
                Write-Host "Could not found the application's (destination) folder physican path. Settings could not be restored.";
			    $settingsRestore = 1;
            }
		}
		else{
...
# Returns true if it successfully unistalled or else false. User must manually uninstall.
# 
function UninstallApplication
{
    ...
    $success = "true";
    write-host "Try to uninstall the application";
	try{
		#$userName = $env:USERPROFILE.Split("\")[-1];
		...
		if($packages -ne $null){
			foreach($package in $packages){	
				...
				#write-host $package;
				write-host "Application successfully unistalled: " $package.PackageFullName;
			}
		}
	}
	catch{
		write-host "Could not uninstall the " $AppName ". Please do so manually and then run the installation again.";
		$ErrorMessage = $_.Exception.Message;
		write-host $ErrorMessage;
        $success = "false";
	}
...
function CheckIfOldVersionInstalled ($appName)
{
	write-host $appName;
	$choice = "y";
	# if error equal to 1 and return back
	...
	try{
		...
		if($packages -ne $null){
			...
			write-host "Found old version of the app";
		}
		else{
			$oldAppExists = 1;
			write-host "Not Found old version of the app";
		}
	}
	catch{
		$oldAppExists = 1;
		write-host "Catch. Found old version of the app";
		$ErrorMessage = $_.Exception.Message;
		write-host $ErrorMessage;
	}    
    if ($oldAppExists -eq 1){
        return $oldAppExists;
    }
...
else
{
	$backUpDBOld = -1;
	$backUpDBNew = -1;
	# Check if old version of the app is install to the machine.
	$checkIfOldAppExists = CheckIfOldVersionInstalled "46402299-c7d3-43a9-91a3-e45ba5b55f2d";
	
	if ($checkIfOldAppExists -eq 0)
	{
		$backUpDBOld = BackUpDatabase "46402299-c7d3-43a9-91a3-e45ba5b55f2d" "osce_local_db.db";
		write-host $backUpDBOld " 46402299-c7d3-43a9-91a3-e45ba5b55f2d";
		...
		$backUpSettings = BackUpSettings "46402299-c7d3-43a9-91a3-e45ba5b55f2d" "settings.dat";
		Write-Host $backUpSettings;
		if ($backUpSettings -eq 2){
			Write-Host "Installation aborted.";
			Read-host "Press 'enter' to exit";
			Break;
		}
	...
	if ($backUpDBOld -ne 0){
		# Did not copy DB from old app. Check if we can back up new app's DB.
		$backUpDB = BackUpDatabase "OSCEmark" "osce_local_db.db";
		Write-Host $backUpDB;
		if ($backUpDB -eq 2){
			Write-Host "Installation aborted.";
			Read-host "Press 'enter' to exit";
	...
	if ($backUpSettings -ne 0){
		$backUpSettings = BackUpSettings "OSCEmark" "settings.dat";
		Write-Host $backUpSettings;
		if ($backUpSettings -eq 2){
			Write-Host "Installation aborted.";
			Read-host "Press 'enter' to exit";
	...
	if ($checkIfOldAppExists -eq 0)
	{
		$successUninstall = UninstallApplication "46402299-c7d3-43a9-91a3-e45ba5b55f2d";
		Write-Host $successUninstall;
	}
	
	$successUninstall = UninstallApplication "OSCEmark";
	Write-Host $successUninstall;
	
	DoStandardOperations
	
	$restoreDB = RestoreDatabase "OSCEmark" "osce_local_db.db";
	write-host $restoreDB;
	
	$restoreSettings = RestoreSettings "OSCEmark" "settings.dat";
	write-host $restoreSettings;
